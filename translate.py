    ...
    for i in range(0, len(rna_sequence), 3):
        ...
        if codon in genetic_code:
            if genetic_code[codon] == '*':
                ...
                translated_sequence += genetic_code[codon]
    return translated_sequence
    
    #This is failing only 1 test where a single codon is lower case.
    #Make sure to convert your codon to upper case prior to using the dictionary (which is case sensitive).
...
def get_all_translations(rna_sequence, genetic_code):
    ...
    #Right now this is translating the entire sequence, and not starting only with a start codon. 
...
print("Reverse DNA sequence:", get_reverse(dna)) 
   ...
   """Reverse orientation of `sequence`.
    ...
    'CGUA'
    """
    #The best format for defining a script is to have the doc string immediately after the def line. Then you would put the code below this docstring.
    #Additionally if you run the test for this function it is failing one test where it expects 'CGUA' but got 'cgua'
    #Add a line in here that will make the entire sequence uppercase to get it to pass the tests. 
...
def reverse_and_complement(sequence):
    ...
    'GCAU'
    """
    reverse = get_reverse(sequence)
    return get_complement(reverse)
